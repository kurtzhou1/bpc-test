...
const RuleAdd = ({
  ...
  const infoCheck = () => {
    if (ruleName === '') {
      dispatch(
        setMessageStateOpen({
          messageStateOpen: { isOpen: true, severity: 'error', message: '請輸入規則英文名稱' },
        }),
      );
      return false;
    }
    if (ruleCName === '') {
      dispatch(
        setMessageStateOpen({
          messageStateOpen: { isOpen: true, severity: 'error', message: '請輸入規則中文名稱' },
        }),
      );
      return false;
    }
    if (partyName === '' && value === 0) {
      dispatch(
        setMessageStateOpen({
          messageStateOpen: { isOpen: true, severity: 'error', message: '請選擇會員名稱' },
        }),
      );
      return false;
    }
  ...
  const addList = (v) => {
    ...
    if (emailRule.test(v)) {
      ...
      dispatch(
        setMessageStateOpen({
          messageStateOpen: { isOpen: true, severity: 'error', message: '請輸入正確email格式' },
        }),
      );
      setEmailText('');
    }
      ...
      if (infoCheck()) {
        ...
        fetch(addBillNotifyRule, { method: 'POST', body: JSON.stringify(tmpArray) })
          ...
          .then(() => {
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'success', message: '新增規則成功' },
              }),
            );
            handleAddRuleClose();
            formInitial();
      ...
      if (action === 'Add') {
        ...
        fetch(updateSysInvNotifyRule, { method: 'POST', body: JSON.stringify(tmpArray) })
          ...
          .then(() => {
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'success', message: '編輯規則成功' },
              }),
            );
            handleAddRuleClose();
            formInitial();
