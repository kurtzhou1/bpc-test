...
const CorrespondenceMake = ({ isDialogOpen, handleDialogClose, payDraftID }) => {
  ...
  const saveToSend = () => {
    ...
    fetch(getPayDraftStreamCBRefund, { method: 'POST', body: JSON.stringify(tmpArray) })
      .then((res) => res.json())
      .then(() => {
        dispatch(
          setMessageStateOpen({
      ...
      .catch((e) =>
        dispatch(
          setMessageStateOpen({
            messageStateOpen: { isOpen: true, severity: 'error', message: '送出不成功' },
          }),
        ),
      );
    handleDialogClose();
        ...
        <Grid container spacing={2} className="no-print">
          ...
          <Grid item xs={5} sm={5} md={5} lg={5}>
            <Typography sx={{ fontFamily: 'DFKai-sb', fontWeight: 'bold' }}>
              ...
              <Box sx={{ fontSize: '12px' }}>
                &nbsp;&nbsp;&nbsp;&nbsp;Wire/Routing: {correspondenceInfo?.WireRouting}
              </Box>
