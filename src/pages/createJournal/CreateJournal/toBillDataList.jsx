    ...
    fetch(tmpQuery, { method: 'GET' })
      ...
      .then((data) => {
        console.log(
          'data=>>',
          data,
          Array.isArray(data.InvoiceDetail),
          Array.isArray(data.InvoiceMaster),
        );
        let tmpAmount = 0;
        if (Array.isArray(data.InvoiceDetail) && Array.isArray(data.InvoiceMaster)) {
          ...
          dispatch(
            setMessageStateOpen({
              messageStateOpen: { isOpen: true, severity: 'info', message: '沒有攤分資料' },
            }),
          );
        }
      })
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {toBillDataInfo.map((row, id) => {
                  ...
                  return (
                    <TableRow
                      key={row.FeeAmountPre + row?.PartyName + row?.LBRatio + id}
