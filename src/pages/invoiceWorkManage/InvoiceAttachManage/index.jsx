    ...
    if (action === '待立帳') {
      ...
      fetch(updateInvoice, { method: 'POST', body: JSON.stringify(tmpArray) })
        ...
        .then(() => {
          dispatch(
            setMessageStateOpen({
              messageStateOpen: { isOpen: true, severity: 'success', message: 'Validated成功' },
            }),
          );
          setPage(0);
          setAction('');
      ...
      fetch(afterBilled, { method: 'POST', body: JSON.stringify(tmpArray) })
        ...
        .then((data) => {
          if (!data.ifReturn && data.hasOwnProperty('ifReturn')) {
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'error', message: '作廢失敗' },
              }),
            );
            ...
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'success', message: '作廢成功' },
              }),
            );
          }
        })
      ...
      fetch(returnToValidated, { method: 'POST', body: JSON.stringify(tmpArray) })
        ...
        .then((data) => {
          if (!data.ifReturn && data.hasOwnProperty('ifReturn')) {
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'error', message: '退回失敗' },
              }),
            );
            returnDataList.current = data.BillMaster;
            setIsReturnOpen(true);
            ...
            dispatch(
              setMessageStateOpen({
                messageStateOpen: { isOpen: true, severity: 'success', message: '退回成功' },
              }),
            );
          }
        })
        ...
        .then(() => {
          ...
          fetch(deleteInvoiceWKDetail, { method: 'POST', body: JSON.stringify(tmpArray) })
            ...
            .then(() => {
              dispatch(
                setMessageStateOpen({
                  messageStateOpen: { isOpen: true, severity: 'success', message: '刪除成功' },
                }),
              );
              setPage(0);
              setAction('');
